...
A production-grade URL shortener built in Go, with PostgreSQL persistence, Redis caching, async analytics processing, and end-to-end observability via Sentry.
...
# {"short_code":"3d7","short_url":"http://localhost:8080/3d7","long_url":"https://example.com"}
```

Visit it — instant 302 redirect, sub-millisecond on cache hit:

```bash
curl -i http://localhost:8080/3d7
# HTTP/1.1 302 Found
...
| Layer          | Choice                          | Why                                                              |
| -------------- | ------------------------------- | ---------------------------------------------------------------- |
| Language       | Go 1.23+                        | Concurrency primitives, single binary, fast startup              |
...
| Cache          | Redis 7                         | In-memory key-value, atomic ops                                  |
...
**HTTP layer.** Gin router with a middleware chain: Sentry tracing → recovery → optional/required auth → handler.
...
**Fail-open on cache errors, fail-closed on auth errors.** If Redis is unreachable, redirects fall back to the DB with a logged warning — the user notices slightly slower latency, not an outage. Auth errors do the opposite: any token problem is a hard 401, never a silent pass-through.
...
- **Unit tests** with hand-rolled mocks — service logic, base62 encode/decode, JWT generation/validation, worker pool behaviors including drop-on-overflow and graceful shutdown drain
...
- **Rate limiting middleware.** Token bucket with Redis `INCR`+`EXPIRE` per IP/API key.
...
- **Next.js dashboard.** UI for the existing API: register/login, paste URLs, view click charts.
- **Deployment.** Railway or Fly.io for the backend, Vercel for the frontend.
...
- **System design trade-offs** — read-heavy caching, async event processing, fail-open vs fail-closed, the dual-trigger flush pattern (size OR time)
...
- **Operational concerns** — structured logging with `slog`, distributed tracing with manual spans, privacy-conscious error reporting, bounded graceful shutdown
